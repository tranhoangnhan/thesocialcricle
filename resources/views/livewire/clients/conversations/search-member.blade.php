<div>
    <div>
            ...
            <div class="flex items-center flex-wrap">
                ...
                <div class="flex ml-2">
                    <input type="text" id="floating_standard"
                        ...
                        wire:keydown.enter="resetSearch" autocomplete="no" value="" placeholder="Tìm " />
                </div>
            </div>
        </div>
        ...
        @if ($showdiv && !empty($search))
            <div
                class="user-container mt-3 flex flex-col dark:bg-white-800 bg-gray-100 rounded-lg max-h-60 w-100 overflow-y-auto p-2 z-10"  data-simplebar>
                @if (isset($data) && $data->isNotEmpty())
                    @foreach ($data as $friend)
                        ...
                        <div wire:click.prevent="add('{{ $friendId }}')"
                            class="flex items-center p-2 {{ in_array($friendFullName, array_column($tags, 'friendFullName')) ? 'bg-blue-200 text-white' : 'hover:bg-blue-100 focus:outline-none' }}">
                            {!! getAvatar($friendId) !!}
                            <div class="ml-4">
        ...
        <button type="button" wire:click="addToGroup" class="mt-3 bg-blue-600 font-semibold p-2 rounded-md text-center text-white ">
            Thêm
        </button>
